...
#
# Usage:
#   ssh matti@192.168.2.212 "powershell -ExecutionPolicy Bypass -File C:\Users\matti\Desktop\prediction-market-analysis\health_check.ps1"
...
if (Test-Path $wlog) {
    $wlogTime = (Get-Item $wlog).LastWriteTime
    $age = (Get-Date) - $wlogTime
    $ageMin = [int]$age.TotalMinutes
    Write-Host "`n--- Watchdog ---"
    $msg = "watchdog_paper.log last write: " + $wlogTime + "  [" + $ageMin + " min ago]"
    Write-Host $msg
    if ($ageMin -gt 60 -and $nLoop -lt 2) {
...
if (Test-Path $sw) {
    $swTime = (Get-Item $sw).LastWriteTime
    $age = (Get-Date) - $swTime
    $ageMin = [int]$age.TotalMinutes
    Write-Host "`n--- PAPER data writes ---"
    $msg = "skipped_windows.csv last write: " + $swTime + "  [" + $ageMin + " min ago]"
    Write-Host $msg
    if ($ageMin -gt 30) {
        Write-Host "WARN: skipped_windows.csv stale - PAPER may be stuck" -ForegroundColor Yellow
    }
}
...
foreach ($asset in @("BTC", "ETH", "SOL")) {
    ...
    if (Test-Path $f) {
        ...
        if ($age.TotalDays -gt 1) {
            $ageStr = ("{0:N1}d" -f $age.TotalDays)
        } else {
            $ageStr = ("{0:N0}m" -f $age.TotalMinutes)
        }
...
if (Test-Path $blog) {
    ...
    Write-Host ("last write: " + $ageSec + "s ago")
