"""
Sports fade-bottom-whale bot for Polymarket (NHL, NBA, MLB, Tennis, Soccer).

Mirror of esports_fade_bot.py adapted for sports markets. PAPER MODE ONLY —
this bot exists for week-long validation that the fade-losers strategy works
on sports the same way it does on esports. Does NOT place real orders.

Once validated, the consolidation step is to either:
  (a) port LIVE logic into here and run separately at a small bet size, or
  (b) generalize esports_fade_bot.py to handle both categories.

For now: paper data collection only. No --live flag accepted.
"""
...
# Sports bot intentionally silent on Telegram — PAPER mode, no real money,
# no need to ping the user. Override the notify import with a no-op.
...
ES_DIR = ROOT / "cowork_snapshot" / "sports"      # repurposed name — points to sports
...
class FadeBot:
    ...
    def _load_market_index(self):
        ...
        try:
            ...
            print(f"[sports-bot] market index preload failed: {e}")
    ...
    def get_market(self, condition_id: str) -> dict | None:
        ...
        try:
            ...
            print(f"[sports-bot] market fetch error {condition_id[:10]}: {e}")
        return None
    ...
    def _load_targets(self) -> set[str]:
        ...
        print(f"[sports-bot] loaded {len(fade)} fade + {len(self.follow_wallets)} follow wallets")
        return fade
    ...
    def refresh_daily_risk(self) -> None:
        ...
        try:
            ...
            print(f"[sports-bot] daily_risk refresh failed: {e}")
    ...
    def maybe_reload_daily_pnl(self):
        ...
        try:
            ...
            print(f"[sports-bot] daily_pnl reload failed: {e}")
    ...
    def maybe_reload_targets(self):
        ...
        if follow_mtime > getattr(self, "follow_mtime", 0):
            try:
                ...
                print(f"[sports-bot] reloaded {len(wallets)} follow targets "
                      f"(+{added} new, -{removed} dropped)")
                self.write_event({"type": "follow_targets_reloaded",
                                  "count": len(wallets), "added": added, "removed": removed})
            except Exception as e:
                print(f"[sports-bot] follow reload failed: {e}")
    ...
    def process_trade(self, t: dict):
        ...
        # (we'd pay >$1.00 total for a $1.00 payout). Mirrors the same fix
        # in esports_fade_bot.py (2026-05-24).
        ...
        print(f"[sports-bot] {datetime.utcnow().isoformat(timespec='seconds')}Z  "
        ...
        if self.live:
            ...
            print(f"[sports-bot]   DRY-LIVE would post BUY {price}x{shares} "
    ...
    def place_live_order(self, trade: dict):
        ...
        try:
            ...
            print(f"[sports-bot]   LIVE order posted: id={order_id} status={init_stat} price={price} shares={shares}")
            ...
            response_at = time.time()
            err_str = str(e)
            print(f"[sports-bot]   LIVE order POST FAILED: {e}")
        ...
        if order_id:
            ...
            if final_status not in ("matched", "cancelled") and final_matched < shares:
                try:
                    ...
                    print(f"[sports-bot]   LIVE order CANCELLED id={order_id} matched={final_matched:.2f}/{shares:.2f}")
                except Exception as e:
                    print(f"[sports-bot]   LIVE cancel FAILED id={order_id}: {e}")
                    self.write_event({"type": "live_order_cancel_error",
                                      "order_id": order_id, "error": str(e)})

        final_at = time.time()
        cost = round(final_avg_price * final_matched, 4)
        print(f"[sports-bot]   LIVE final: id={order_id} status={final_status} "
    ...
    def take_profit_sweep(self):
        ...
        try:
            ...
            print(f"[sports-bot] tp_sweep positions fetch failed: {e}")
            return
        ...
        for p in open_positions:
            ...
            try:
                ...
                print(f"[sports-bot]   TP SELL {sell_size}@{sell_price} {outcome[:18]:>18}  id={oid[:18]}... status={status}")
                ...
                print(f"[sports-bot]   TP SELL FAILED for {p.get('outcome','?')}: {e}")

    def run(self):
        print(f"[sports-bot] polling every {POLL_INTERVAL}s — writing to {OUT_DIR}")
        # Initialize daily_risk_usd from actual matched orders today
        if self.live:
            self.refresh_daily_risk()
            print(f"[sports-bot] daily_risk_usd initialized: ${self.daily_risk_usd:.2f}")
        ...
        try:
            while True:
                ...
                if time.time() - last_summary > 60:
                    ...
                    print(f"[sports-bot] heartbeat: trades_scanned={n_trades_seen} fades={n_fades_now} "
                    ...
                    if n_fades_now > last_fades_value:
                        ...
                        if stall_active:
                            ...
                            print(f"[sports-bot] STALL RECOVERED after {stall_duration:.0f}s "
                        ...
                        if (not stall_active
                            ...
                            print(f"[sports-bot] !!! SIGNAL STALL DETECTED !!! "
